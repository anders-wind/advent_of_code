...
auto find_max_three_inventory(const std::vector<std::vector<int64_t>>& input) -> int64_t
{
    auto sorted =
        std::ranges::sort(input | std::ranges::views::transform(
                                      [](const std::vector<int64_t>& list)
                                      { return std::accumulate(list.begin(), list.end(), 0LL); }));
    auto max_three = sorted | std::ranges::views::reverse | std::ranges::views::take(3);

    // std::ranges dont have
    auto accum = 0LL;
    for (auto elem : max_three)
    {
        accum += elem;
    }
    return accum;
