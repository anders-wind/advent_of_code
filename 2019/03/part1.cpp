...
std::tuple<std::vector<LineX>, std::vector<LineY>> play_moves(const std::vector<Move>& moves, const Coord& start) {
    ...
    for(const auto& move : moves) {
        if(move.direction == 'U') {
            ...
            line_ys.push_back(LineY{current_position.y, new_y, current_position.x});
            ...
            line_xs.push_back(LineX{current_position.x, new_x, current_position.y});
...
int main()
{
    ...
    for(auto& line_x : line1_xs) {
        for(auto& line_y: line2_ys) {
            ...
            if(intersect) {
                auto distance = manhatten_distance(intersect.value(), start);
